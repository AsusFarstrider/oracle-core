...
def check_music_health(*, music_execution=None) -> MusicHealthResponse:
    from .music import check_music_health as build_music_health

    return MusicHealthResponse(**build_music_health(music_execution=music_execution))


def check_calendar_health(*, canonical_execution=None) -> CalendarHealthResponse:
...
def check_news_health(*, canonical_execution=None) -> NewsHealthResponse:
...
def check_tts_health(*, provider=None) -> TtsHealthResponse:
    if provider is None:
    ...
    return TtsHealthResponse(
        status=("disabled" if not status.configured else "ok" if status.available else "failed"),
        service="oracle-brain",
...
def check_stt_health(*, provider=None) -> SttHealthResponse:
    if provider is None:

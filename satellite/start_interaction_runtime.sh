...
if [[ "$#" -gt 1 || ( "$#" -eq 1 && "$1" != "--list-devices" ) ]]; then
  echo "canonical interaction launcher rejects behavior arguments" >&2
  exit 2
fi
for name in ORACLE_SATELLITE_ID ORACLE_SATELLITE_PROJECTION_STORE_ROOT ORACLE_SATELLITE_RUNTIME_COMPATIBILITY_PATH; do
  if [[ -z "${!name:-}" ]]; then
    echo "canonical interaction launcher requires ${name}" >&2
    exit 2
  fi
done
...
if [[ -z "${PIPEWIRE_RUNTIME_DIR:-}" && -S "${XDG_RUNTIME_DIR}/pipewire-0" ]]; then
...
exec "${python_bin}" "${satellite_dir}/pi_wake_satellite.py" "$@"

#!/usr/bin/env python3
"""Run the bounded Stage 7 fallback corpus against Luna and/or local Phi."""
...
from oracle_app.constants import FALLBACK_ROUTER_SYSTEM_PROMPT  # noqa: E402
from oracle_app.handlers.fallback_router import (  # noqa: E402
    FALLBACK_ROUTER_RESULT_SCHEMA,
    validate_fallback_router_result,
)
...
DEFAULT_CORPUS = ROOT / "tests" / "fixtures" / "fallback_inference_corpus.json"
...
def _client(provider_id: str, provider: InferenceProviderSettings, total_timeout: float) -> InferenceClient:
    return InferenceClient(
        InferenceExecutionSettings(
            ...
            fallback_model=provider.model if provider.provider_type == "ollama" else None,
            fallback_timeout_seconds=provider.timeout_seconds if provider.provider_type == "ollama" else None,
            local_provider_id=provider_id if provider.provider_type == "ollama" else None,
            providers={provider_id: provider},
            consumer_orders={"fallback_router": (provider_id,)},
            consumer_total_timeout_seconds={"fallback_router": total_timeout},
        )
    )


def _matches(case: dict[str, Any], decision: dict[str, str]) -> tuple[bool, list[str]]:
    problems: list[str] = []
    allowed_statuses = tuple(case.get("allowed_statuses") or (case["expected_status"],))
    if decision["status"] not in allowed_statuses:
        problems.append(f"status:{decision['status']}")
    if decision["domain"] != case.get("expected_domain", ""):
        problems.append(f"domain:{decision['domain']}")
    expected_user = case.get("expected_user_id")
    if expected_user is not None and decision["user_id"] != expected_user:
        problems.append(f"user_id:{decision['user_id']}")
    normalized = decision["normalized_text"].lower()
    for term in case.get("required_terms", []):
        if str(term).lower() not in normalized:
...
def run_evaluation(
    *,
            ...
            for case in corpus["cases"]:
                ...
                try:
                    execution = client.execute(
                        ...
                        validate=lambda raw, original=case["input"]: validate_fallback_router_result(
                            raw, original_text=original
                        ),
                    )
                    latency_ms = round((perf_counter() - started) * 1000, 1)
                    passed, problems = _matches(case, execution.value)
                    results.append(
                        {
    ...
    return {
        ...
        "prompt_sha256": hashlib.sha256(FALLBACK_ROUTER_SYSTEM_PROMPT.encode("utf-8")).hexdigest(),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.case_id:
        ...
        missing = sorted(selected - {case["id"] for case in corpus["cases"]})
        if missing:
